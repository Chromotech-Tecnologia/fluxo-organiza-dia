...
export function TaskCardImproved({
  ...
  return <Card ref={setNodeRef} style={style} className={`cursor-pointer hover:shadow-md transition-all duration-200 w-full min-w-0 ${getCardColor()}`} onClick={handleCardClick}>
      <CardContent className="p-2 md:p-3 min-w-0">
        <div className="space-y-2 md:space-y-3">
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="end" className="w-40">
                  {onEdit && <DropdownMenuItem onClick={handleEditClick}>
          ...
          <div className="flex gap-1 flex-wrap">
            ...
            {task.priority !== 'none' && <Badge variant="outline" className={`text-xs h-5 ${getPriorityColor(task.priority)}`}>
                {task.priority === 'extreme' ? 'Extrema' : 'Prioridade'}
              </Badge>}

            {task.type === 'meeting' && <Badge variant="outline" className="text-xs h-5 bg-blue-100 text-blue-800 border-blue-300">
                <Calendar className="h-2.5 w-2.5 mr-1" />
                Reunião
              </Badge>}
            
            {task.type === 'delegated-task' && <Badge variant="outline" className="text-xs h-5 bg-purple-100 text-purple-800 border-purple-300">
                <User className="h-2.5 w-2.5 mr-1" />
                Delegada
              </Badge>}

            {task.forwardCount > 0 && <Badge variant="outline" className="text-xs h-5 bg-yellow-100 text-yellow-800 border-yellow-300">
                <ArrowRight className="h-2.5 w-2.5 mr-1" />
                {task.forwardCount}x
              </Badge>}

            {timeMinutes > 0 && <Badge variant="outline" className="text-xs h-5 bg-indigo-100 text-indigo-800 border-indigo-300">
