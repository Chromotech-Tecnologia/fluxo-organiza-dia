...
export function generateRoutineTasks(taskData: RoutineTaskData): Omit<Task, 'id' | 'createdAt' | 'updatedAt'>[] {
  ...
  return routineDates.map((date, index) => ({
    ...
    isRecurrent: true,
    isRoutine: true,
    routineCycle: taskData.routineCycle,
    routineStartDate: taskData.routineStartDate,
    routineEndDate: taskData.routineEndDate,
    includeWeekends: taskData.includeWeekends,
    completionHistory: [],
    isForwarded: false,
    isConcluded: false,
    attachments: taskData.attachments || []
